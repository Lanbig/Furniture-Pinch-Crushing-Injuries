...
### check Colname ex: checkColname("Assets/DTH)
...
### Load every CSV in the folder ex: df <- bulkLoadCSV("Assets/DEATH")
bulkLoadCSV <- function(pfile, old_data = F) {
  #1999 - 2008p 
  #2009 - 2014
  options( warn = -1 )
  ...
  if(missing(old_data) || old_data == F)
  {
    ...
    df = ldply(listname, fread, header = T, sep = ',' , integer64='character')
    #df['Report_Number'] <- as.character(df['Report_Number'])
  }
